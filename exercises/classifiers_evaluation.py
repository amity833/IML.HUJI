...
def run_perceptron():
    ...
    for n, f in [("Linearly Separable", "../datasets/linearly_separable.npy"),
        ...
        perceptron = Perceptron(callback=lambda p, x, z: losses.append(p.loss(X, y)))

        perceptron.fit(X, y)

        # Plot figure of loss as function of fitting iteration
        loss_vs_iterations = px.line(losses, x=np.arange(len(losses)), y=losses,
                                     title="Loss as function of fitting iteration " + n, )
        loss_vs_iterations.update_xaxes(title='fitting iteration')
        loss_vs_iterations.update_yaxes(title='loss values')
        loss_vs_iterations.show()
        print(len(losses))
...
def compare_gaussian_classifiers():
    ...
    for f in ["../datasets/gaussian1.npy", "../datasets/gaussian2.npy"]:
        ...
        fig = make_subplots(rows=1, cols=2, subplot_titles=(
            "Gaussian Naive Bayes predictions \n| accuracy=" + str(accuracy(y, gnb_pred)),
            "LDA predictions \n| accuracy=" + str(accuracy(y, lda_pred))))

        models = [gnb, lda]
        for i, m in enumerate(models):
            fig.add_traces(go.Scatter(x=X[:, 0], y=X[:, 1], mode="markers", showlegend=False,
                                      marker=dict(color=m.predict(X), symbol=symbols[y], colorscale=[custom[0], custom[-1]],
                                                  line=dict(color="black", width=1))),
                           rows=(i // 3) + 1, cols=(i % 3) + 1)

        fig.update_layout(title_text="Side By Side Subplots LDA algorithm and Gussian naive bayes"+ f)
