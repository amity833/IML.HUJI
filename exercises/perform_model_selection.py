...
def select_polynomial_degree(n_samples: int = 100, noise: float = 5):
    ...
    print("Best fitted k (polinomial degree) = ", min_k, ", noise = ", noise, ",\n Validation error = ",
          min_valid_error, ", Test error = ", test_err, "\n")
...
def select_regularization_parameter(n_samples: int = 50, n_evaluations: int = 500):
    ...
    lambdas = np.linspace(0, 5, n_evaluations)
    ...
    for lam in lambdas:
        ...
        r_t_e, r_v_e = cross_validate(ridge_model, train_X, train_y, mean_square_error)
        ridge_train_error.append(r_t_e)
        ridge_validation_error.append(r_v_e)
        l_t_e, l_v_e = cross_validate(lasso_model, train_X, train_y, mean_square_error)
        lasso_train_error.append(l_t_e)
        lasso_validation_error.append(l_v_e)

    fig7 = go.Figure([go.Scatter(x=lambdas, y=ridge_train_error, mode="markers", name="ridge train errors",
                                 marker=dict(opacity=.7)),
                      go.Scatter(x=lambdas, y=ridge_validation_error, mode="markers", name="ridge validation errors",
                                 marker=dict(opacity=.7)),
                      go.Scatter(x=lambdas, y=lasso_train_error, mode="markers", name="lasso train errors",
                                 marker=dict(opacity=.7)),
                      go.Scatter(x=lambdas, y=lasso_validation_error, mode="markers", name="lasso validation errors",
                                 marker=dict(opacity=.7))
                      ])
    fig7.update_layout(
        title_text=rf"$\text{{Train and Validation errors for LASSO and RIDGE regression models }}$",
        xaxis={"title": r"Lambdas"},
        yaxis={"title": r"Error values", })
    fig7.show()



    # Question 8 - Compare best Ridge model, best Lasso model and Least Squares model
    best_lam_ridge = np.argmin(ridge_validation_error)
    best_lam_lasso = np.argmin(lasso_validation_error)
    ridge = RidgeRegression(lambdas[best_lam_ridge])
    lasso = Lasso(lambdas[best_lam_lasso])
    linear_reg = LinearRegression()
    ridge.fit(train_X, train_y)
    lasso.fit(train_X, train_y)
    linear_reg.fit(train_X, train_y)
    ridge_error = ridge.loss(test_X, test_y)
    lasso_error = mean_square_error(lasso.predict(test_X), test_y)
    linear_reg_error = linear_reg.loss(test_X, test_y)
    print("The best regularization parameter for RIDGE is: ", np.round(lambdas[best_lam_ridge], 3),
          "\nRIDGE TEST ERROR: ", np.round(ridge_error, 3))
    print("The best regularization parameter for LASSO is: ", np.round(lambdas[best_lam_lasso], 3),
          "\nLASSO TEST ERROR: ", np.round(lasso_error, 3))
    print("LINEAR REGRESSION TEST ERROR: ", np.round(linear_reg_error, 3))
