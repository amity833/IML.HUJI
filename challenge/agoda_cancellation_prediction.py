...
def refactor_by_challenge_request(df: pd.DataFrame, fild_factor_1, fild_factor_2, fild_1_dates, fild_2_dates):
    ...
    if len(fild_2_dates) == 2:
        ...
        # df = df[df['cancellation_datetime'] != 'Na']
        # df["cancellation_datetime"] = np.where(df["cancellation_datetime"].isna(), 0, 1)
        df['cancellation_datetime'] = df['cancellation_datetime'].apply(lambda x: 0 if x.type()=NaTType else x)
        df = df[pd.to_datetime(df['cancellation_datetime']).dt.date.between(test_cancellation[0], test_cancellation[1])]
        # df["cancellation_datetime"] = np.where(df["cancellation_datetime"].between(test_cancellation[0],test_cancellation[1]), 1, 0)
    else:
...
def load_data(filename: str, test_train):
    ...
    full_data = full_data.dropna(subset=fields_who_cant_be_null).drop_duplicates()
    clean_data = check_validation(full_data)
    clean_data["is_user_logged_in"] = np.where(clean_data["is_user_logged_in"], 1, 0)
    clean_data["is_first_booking"] = np.where(clean_data["is_first_booking"], 1, 0)
    clean_data["charge_option_1"] = np.where(clean_data["charge_option"] == 'Pay Later', 1, 0)
    clean_data["charge_option_2"] = np.where(clean_data["charge_option"] == 'Pay Now', 1, 0)

    for field in dates_fields_to_format:
        format_datetime_to_date(clean_data, field)
    if test_train == 'train':
        # clean_data["cancellation_datetime"] = np.where(clean_data["cancellation_datetime"].isna(), 0, 1)
        # clean_data= clean_data[clean_data['cancellation_datetime'] != 'Na']
        # clean_data= clean_data[
        #     datetime.strptime("2018-12-07", '%Y-%m-%d').date() <= clean_data['cancellation_datetime'] <= datetime.strptime('2018-12-13', '%Y-%m-%d').date()]
        data_after_filtration = refactor_by_challenge_request(clean_data, 'checkout_date', 'booking_datetime',
                                                              train_checkout,
                                                              train_booking)
        ...
        clean_data = clean_data[clean_data['accommadation_type_name'] == "Hotel"]
        # clean_data = clean_data[clean_data['h_customer_id'] >= 0]
        clean_data = clean_data[~clean_data['cancellation_policy_code'].isin(non_valid_cancellation_codes)]
        data_after_filtration = refactor_by_challenge_request(clean_data, 'booking_datetime', 'checkin_date',
                                                              test_booking, test_checkin)

        data_after_filtration = data_after_filtration[data_after_filtration['checkout_date'] >= test_cancellation[0]]

        return data_after_filtration[filter_list]
...
if __name__ == '__main__':
    ...
    estimator.loss(train_X.values, train_y.values)
