...
def refactor_by_challenge_request(df: pd.DataFrame, fild_factor_1, fild_factor_2, fild_1_dates, fild_2_dates):
    # field_factor 1 = checkout, field factor 2 = booking, field 1 date =
...
def load_data(filename: str, test_train):
    ...
    if test_train == 'train':
        # data["cancellation_datetime"] = np.where(data["cancellation_datetime"].isna(), 0, data["cancellation_datetime"])
        data_after_filtration = refactor_by_challenge_request(data, 'checkout_date', 'booking_datetime',
                                                              train_checkout,
                                                              train_booking)
        data["cancellation_days"] = np.where(data['cancellation_datetime'].isna(),
                                                              data['checkout_date'], data['cancellation_datetime'])
        data["cancellation_days"] = (data["cancellation_days"] - data["checkin_date"]).dt.days
        # data_after_filtration["cancellation_days"] = np.where(data_after_filtration["cancellation_datetime"].isna(), 0, 1)
        features = data_after_filtration[filter_list]
        labels = data["cancellation_days"]
...
if __name__ == '__main__':
    ...
    # Load data
    df, cancellation_days_labels = load_data("../datasets/agoda_cancellation_train.csv", 'train')
    train_X, train_y, test_X, test_y = split_train_test(df, cancellation_days_labels)
    # Fit model over data
